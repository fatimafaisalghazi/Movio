...
class AddRatingSeriesUseCaseTest {
    ...
    @Test
    fun `invoke SHOULD call repository addRatingSeries with movieId and rate`() = runTest {
        val movieId = 1
        val rate = 4.5
    ...
    @Test
    fun `invoke SHOULD call repository addRatingSeries with minimum rate`() = runTest {
        val movieId = 123
        val rate = 0.0
        coEvery { seriesRepository.addRatingSeries(movieId, rate) } returns Unit
    ...
    @Test
    fun `invoke SHOULD call repository addRatingSeries with maximum rate`() = runTest {
        val movieId = 456
        val rate = 10.0
        coEvery { seriesRepository.addRatingSeries(movieId, rate) } returns Unit
    ...
    @Test
    fun `invoke SHOULD call repository addRatingSeries with negative movieId`() = runTest {
        val movieId = -1
        val rate = 3.5
        coEvery { seriesRepository.addRatingSeries(movieId, rate) } returns Unit
    ...
    @Test
    fun `invoke SHOULD call repository addRatingSeries with decimal rate`() = runTest {
        val movieId = 789
        val rate = 7.25
        coEvery { seriesRepository.addRatingSeries(movieId, rate) } returns Unit
